...
def main():
    ...
    file_path = os.path.join(folder_path, "text_features_mean.parquet")
    data = ParquetDataHandler(file_path).load().get()
    train_x = train_x.join(data)
    test_x = test_x.join(data)
    import pdb;
    pdb.set_trace()
    ...
    print(f"describe:\n{preds.describe()}")
    preds.to_csv("../result/submission_baseline_ext.csv")
